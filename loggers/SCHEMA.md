...
3. `log_if_env` - Another conditional log function, though this only logs if the `CODECTRL_DEBUG` is set. Should take 4 total parameters: `message : T` (required), `surround : uint` (optional), `host : string` (optional), `port : string/int` (optional).
